...
describe Player do
  ...
  describe :with_team do
    ...
    let!(:players_with_one_team) { create_list(:player, 10) }
    before { team.players << players_with_one_team }

    it "should return players scoped to specified team_id" do
      expect(Player.with_team(players_with_one_team.last.teams.first)).to match_array(players_with_one_team)
    end
  end
end
